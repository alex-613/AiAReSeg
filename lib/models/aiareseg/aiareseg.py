...
from .head import build_box_head, build_iou_head,build_segmentation_head
...
class AIARESEG(BASIC):
    """
    This is the base class for Transformer Tracking.
    """

    def __init__(self, backbone, transformer, box_head, num_queries,
                 aux_loss=False, head_type='CORNER', iou_head=None, mask_head=None):
        """
        Initializes the model.
    ...
    def forward(self, img=None, search_dic=None, refer_dic_list=None, refer_reg_list=None, out_embed=None,
                proposals=None, mode='backbone', search_outputs=None, reference_outputs=None):
        if mode == 'backbone':
            ...
            return self.forward_segmentation(out_embed, search_outputs=search_outputs, reference_outputs=reference_outputs)
        else:
            raise ValueError
    ...
    def forward_segmentation(self, hs,search_outputs=None, reference_outputs=None):

        mask_output = self.mask_head(hs, search_outputs=search_outputs, reference_outputs=reference_outputs)
        return mask_output
    ...
    def adjust(self, output_back: list, pos_embed: list, inr_embed: list):
        ...
        # New, output the larger channels for segmentation task
        seg_channel160, mask160 = output_back[0].decompose()
        seg_channel80, mask80 = output_back[1].decompose()
        seg_channel40, mask40 = output_back[2].decompose()
        seg_channel20, mask20 = output_back[3].decompose()

        return {'feat': feat_vec, 'mask': mask_vec, 'pos': pos_embed_vec, 'inr': inr_embed_vec, 'Channel160': seg_channel160, 'Channel80': seg_channel80, 'Channel40':seg_channel40,'Channel20': seg_channel20}


def build_aiareseg(cfg, segmentation=False):
    """
    ...
    mask_head = build_segmentation_head(cfg)

...
from lib.models.aiareseg.attention import AiAModule
...
class TransformerEncoder(nn.Module):
    ...
    def forward(self, src,
        ...
        # fig, axs = plt.subplots(1, len(self.layers) + 1)
        ...
        #
        # axs[0].imshow(plot_img_transformed)


        for stack, layer in enumerate(self.layers, 1):
            output = layer(output, src_key_padding_mask=src_key_padding_mask, pos=pos, inr=inr)

            # TODO: Lets try to plot the output attention maps here, for now we will only use the first hidden dim, but we can use other ones later

            # # output reshaping
            # copy_output = output
            # copy_output = copy_output.permute(1, 2, 0).view(-1, 256, 20, 20)
            #
            # # positional embedding
            # copy_embed = pos
            # copy_embed = copy_embed.permute(1, 2, 0).view(-1, 256, 20, 20)
            # plot_feature_tensor = copy_output * copy_embed
            #
            # # Before detaching, we need to "denormalize" the pixels, or at least scale it back to 0 and 1
            # plot_feature = plot_feature_tensor[0, 0, ...].detach().cpu().numpy().astype(float)
            # min_val = np.min(plot_feature)
            # max_val = np.max(plot_feature)
            #
            # new_min = 0.0
            # new_max = 1.0
            #
            # plot_feature_transformed = new_min + ((plot_feature - min_val) * (new_max - new_min)) / (max_val - min_val)
            #
            # # Plotting image
            # axs[stack].imshow(plot_feature_transformed)

...
class PositionEmbeddingSine(nn.Module):
    ...
    def __init__(self, num_pos_feats=64, temperature=10000, normalize=False, scale=None):
        ...
        # Default scale is 2pi for the sine encoding
        if scale is None:
            scale = 2 * math.pi
        self.scale = scale
    ...
    def forward(self, tensor_list: NestedTensor):
        """
        The forward pass.
        We are passing in a tensor list, aka the feature maps in to the positional encoding
        """
        # (b,d,h,w)
        ...
        # num_pos_feats = d/2
        # Creates a 1D tensor from 0 to the num_pos_feats
        ...
        # Set the x and y positional encodings by stacking the sine function and the cos functions
...
class PositionEmbeddingLearned(nn.Module):
    ...
    def forward(self, tensor_list: NestedTensor):
        ...
        # Now we would like to repeat the x embedding h times before
        pos = torch.cat([
...
def build_position_encoding(cfg):
    """
    Implement positional encoding in the network.
    The encoding can be the standard sine function, or with learned parameters

    Args:
        cfg (class:edict): This is an edict that contains all of the settings you need to build the model

    Returns:
        positional_embedding (class): This is the positional embedding that is used to encode each element of the array
        inner_positional_embedding (class): This is the positional embedding for the inner attention loop (AiA).
    """

    # The hidden dimension is 256, we are halving it, so the positional encoding is applies every two hidden dimensions
    N_steps = cfg.MODEL.HIDDEN_DIM // 2
    # Option 1: Apply the sine positional embedding
    if cfg.MODEL.POSITION_EMBEDDING in ('v2', 'sine'):
        position_embedding = PositionEmbeddingSine(N_steps, normalize=True)
    # Option 2: Apply the learned positional embedding

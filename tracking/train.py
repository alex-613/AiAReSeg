...
def parse_args():
    ...
    parser.add_argument('--nnodes', type=int, help='number of nodes used')
...
def main():
    ...
    args = parse_args()
    if args.mode == 'single':
        train_cmd = 'python lib/train/run_training.py --script %s --config %s --save_dir %s --segmentation %s' \
                    % (args.script, args.config, args.save, args.segmentation)
        ...
        train_cmd = 'python -m torch.distributed.launch --nproc_per_node %d' \
                    ...
                    '--script %s --config %s --save_dir %s --segmentation %s' \
                    % (args.nproc, args.script, args.config, args.save, args.segmentation)
    else:
